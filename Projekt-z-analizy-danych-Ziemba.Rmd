...
##Podział zbioru
```{r zbiortestowy, echo=FALSE, message=FALSE}
library(caret)
library(dplyr)
zbior<-summarize(group_by(dane,ora, idsito), energia=sum(kwh,na.rm=TRUE))
set.seed(23)
inTraining <- 
    createDataPartition(
        y = zbior$energia,
...
training <- zbior[ inTraining,]
testing  <- zbior[-inTraining,]

training
```
##Uczenie
```{r uczenie, echo=FALSE, message=FALSE, warning=FALSE}
ctrl <- trainControl(
    method = "repeatedcv",
    number = 15,
    repeats = 3)
...
##Predykcja
```{r predykcja, echo=FALSE, message=FALSE}

rfClasses <- predict(fit, testing)
RMSE <- sqrt(sum((rfClasses  - testing$energia)^2)/length(rfClasses ))
